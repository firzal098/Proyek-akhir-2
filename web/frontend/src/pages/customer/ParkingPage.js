...
function ParkingPage() {
  ...
  const [checkInData, setCheckInData] = useState({ nomor_polisi: '', slot_parkir: '' });
  ...
  const handleCheckIn = async () => {
    try {
      ...
      setCheckInData({ nomor_polisi: '', slot_parkir: '' });
      loadParkingMap();
      loadActiveTickets();
    } catch (error) {
  ...
  const checkLoyaltyPoints = async () => {
    if (!loyaltyPlate) return;
    try {
      const response = await axios.get(`${API_URL}/parking/loyalty/${loyaltyPlate}`);
        ...
        <Grid item xs={12} md={4}>
          ...
          <Paper sx={{ p: 3, mt: 2 }}>
            ...
            <TextField
              ...
              label="License Plate"
              value={loyaltyPlate}
              onChange={(e) => setLoyaltyPlate(e.target.value)}
              sx={{ mb: 2 }}
            />
            <Button variant="outlined" fullWidth onClick={checkLoyaltyPoints}>
              Check Points
            </Button>
            {loyaltyPoints && (
              <Typography sx={{ mt: 2 }}>
                Points: {loyaltyPoints.poin || 0}
              </Typography>
            )}
          </Paper>
        </Grid>
